...
Ship it:
- **Backend + agent** → a single **AWS EC2 free-tier** instance (`t3.micro`, 1 vCPU / 1 GB RAM) running the backend container behind **Caddy** (automatic HTTPS via Let's Encrypt).
- **Frontend** → **Vercel**.
...
- AWS account (free tier), a hostname pointing to the instance (custom domain, DuckDNS, or `sslip.io`), a GitHub repo, a Vercel account, an Aura Free DB, a Supabase project.
...
### 4. Prod compose on the box — `infra/aws/docker-compose.prod.yml`
...
### 5. EC2 provisioning — `infra/aws/cloud-init.yml`

Launch a `t3.micro` (Amazon Linux 2023 or Ubuntu LTS) in a free-tier region. Allocate + attach an **Elastic IP**. Security group: allow 22 (your IP), 80, 443. Cloud-init (user data) should:
...
### 6. Deploy script — `infra/aws/deploy.sh`
...
- Do NOT use more than 1 uvicorn worker on the free-tier box.
